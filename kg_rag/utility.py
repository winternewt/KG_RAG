...
def interactive(question, vectorstore, node_context_df, embedding_function_for_context_retrieval, llm_type, edge_evidence, system_prompt, api=True, llama_method="method-1"):
    ...
    if llm_type == "llama":
        ...
        chat_id=llm_type if openai.api_type == "azure" else None
        output = get_GPT_response(enriched_prompt, system_prompt, llm_type, chat_id, temperature=config_data["LLM_TEMPERATURE"])        
        stream_out(output)
